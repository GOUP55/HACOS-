...
## 進め方
- まず対象ファイルとNEXT_CHAT_HANDOFF.mdを読む → 最小差分で実装 → ビルド → スクショ確認 → 後片付け
- 公開（main反映）はディレクター/ユーザーの確認後。サンプル・下書きを含む変更はブランチに留める

## 共通の規律（CLAUDE.md「思考の規律」より）
- 記憶で書かない。料金・締切・定員をHTMLに反映するときは `BUSINESS_RULES.md` を確認してから
- 「完了」はビルドと表示確認を済ませてから言う。確認していないことを確認済みのように報告しない
- 直せなかった箇所・飛ばした確認は正直に報告する
