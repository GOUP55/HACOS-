# 次チャット 引き継ぎ（2026-07-15/16 時点・Wellness Journey第1期 募集開始直前）
...
## 🏃 Wellness Journey 7DAYS（7/13〜16・企画担当セッションの引き継ぎ）

**正本**: 価格・締切＝`BUSINESS_RULES.md`「HACOS Wellness Journey」／仕様・決定ログ25件＝`WELLNESS_JOURNEY_MASTER_SPEC.md` v1.0

### 完了（すべてmainマージ済み。PR#39・#41・#47・#48・#49で確認可）
- **価格正式化＆LP公開**: 7DAYS ¥39,800（第1期モニター¥34,800／ふたり割各¥2,000・**併用不可で大きい方**）／30DAYS ¥79,800（食事倍増版）／現金orSTORES／入金3日以内で席確定／締切7/31 → `retreat.html` 公開済み（その後PR#53で別セッションがSEO・カウントダウン・実写・STORESボタン枠を強化）
- **8月シフト確定・チーム共有済み**（`WELLNESS_JOURNEY_STAFFING.md` v1.4）: 朝RUN全てGO／夜クラス水木メイン片山さん・ちひろさん月1〜週1／金土リクエスト制GO／日曜クラス全5回（8/9ちひろ・8/16めぐみ・他GO）
- **決定**: 第1期はフル1本（バリエーション3案は第2期から・PR#48/#49）／食事¥19,800は全額みどり/ふみやさんへ（内訳は2人に一任）／メンバーズ一旦廃止／3段階構成（7DAYS→30DAYS→プレミアム）
- **制作済み資料**: 食事運営表・申込フォーム設計・Day0台本・規約ドラフト＋保健所質問リスト・配信カレンダー6本（①ティザーはオーナー配信OK済み）・ウェルネスレポート設計・食事構成フレーム・8月HMCクラスプラン・ハコさんbot追加パック叩き台（`drafts/wellness-journey-hakosan-pack.md`）

### ⚠️ 重要な最新変更（別セッション発・こちらが正）
- **第1期開催週の第1候補は「8月中旬スタートの週」に変更**（BUSINESS_RULES.md:43・7/17オーナー採用。8/1週だと食事製造締切が申込締切より先に来る矛盾のため）→ `WELLNESS_JOURNEY_MASTER_SPEC.md`・`WELLNESS_JOURNEY_CUSTOMER_JOURNEY.md`・`WELLNESS_JOURNEY_STAFFING.md` 内の「8/1週モデル」表記は**古い**。日付に触る作業をするときはBUSINESS_RULESとretreat.htmlを正とすること（8/9卒業RUN前提のシフト表記も開催週決定後に要追従）

### 未完了（次セッションのタスク候補・優先順）
1. **7/18(土) 募集開始配信**（`WELLNESS_JOURNEY_LAUNCH_CALENDAR.md` ②をコピー・LPリンク付き）→ 以降7/21③・7/24④・7/27⑤・7/30⑥
2. **申込フォームのGoogleフォーム実装**（設計書: `WELLNESS_JOURNEY_ENTRY_FORM.md`。オーナー作業・支援可）
3. **食事の中身待ち**: みどり/ふみやさんの献立返答（`WELLNESS_JOURNEY_MENU_FRAMEWORK.md` のチェックリスト①〜⑤）→ 届いたら食事運営表へ反映
4. トレーナー報酬・GO報酬の数字（面談相談方式・未決定）
5. **メンバーズ廃止の後処理**（リスク#22）: メインLP掲載の撤去＋premium.html卒業生特典（メンバーズ前提）の代替 → **司令塔の担当領域**
6. 保健所確認（オーナー判断で保留中・リスク#13/#25）／法人2系統の体系整理（BUSINESS_RULES.md:50）
7. ハコさんpack: GO赤入れ→companion-persona.mdへ統合→プロンプト再生成→**Harnessセッションへ反映依頼**
8. 申込が入り始めたら: 入金確認→席確定の運用・週次レビュー（売上/申込数/課題）開始
