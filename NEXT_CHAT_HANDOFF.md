...
**残るはLINE予約システムの改修と、オーナー決定が要る3件。まだmainにマージしていない＝本番サイトは旧内容のまま。**
...
**QA（`hacos-qa`）1周実施済み。** 高1・中2・低4の指摘のうち、高1と中2件・低2件を修正。残りは下の「4. QAで残した宿題」。

### ⚠️ この作業中に見つけた設計書の誤り（重要・訂正済み）

**「単品で揃えると¥88,000」は成り立たない。** 設計書の単品合計はセミパを**¥3,000（会員）**として積んでいるが、確定価格は**ドロップイン¥5,000（一律）**。積み直すと**¥104,000**になる。
...
| 商品 | 価格 |
|---|---|
| 朝RUN | 無料（**予約制にする** ⚠️ 公開中のLPは「予約不要」。上の「聞くこと①」参照） |
...
### 1. オーナーに聞くこと（3件・実装より先）

**① 朝RUN（RUNFREE）を予約制にするか。** ここが設計と公開物で食い違っている。
8/11の設計は「前日に人数が読めないので**予約制にする**」。ところが**8/2にmainへ入ったPR #61（RUNFREE主軸のLP刷新・オーナー承認済み）は「予約不要」を打ち出しの柱にしている**（ヒーロー・料金・FAQの計4箇所）。どちらかが引っ込む必要がある。予約制にするならLPのコピーを書き換える話になるので、**LINE予約システムの改修に着手する前に決める**。

**② パーソナルの謝礼を「60分あたり¥3,000」の時間比例にしてよいか。**（8/11から持ち越し）
いまは「1回¥3,000の固定」。10分刻みで時間を選べる設計にする以上、**GOは15分・他トレーナーは20分より短いセッションが必ず赤字**になる（10分ならGO −¥1,000／他 −¥1,500）。時間比例にすれば利益率が一定（GO75%／他67%）。

**③ ジャーニー¥78,000の返金・キャンセル規定。**（新規・募集開始のブロッカー）
特商法にこの節だけ書けなかった。でっち上げずに空けてHTMLコメントで印をつけてある。**¥78,000を売り始める前に必須。** 推奨案は `PRICING_RESTRUCTURE_2026-08.md` のゲートD4（プレミアムの規定に揃える形／食事の扱いだけ切り分ける）。

### 2. 実装の残り

**LINE予約システムの改修**（`line-reservation/`・①の決定後）。回数券の残骸は**4ファイル34箇所**：

| ファイル | 箇所 |
|---|---|
| `liff/reserve.html` | 14（参加区分のラジオ・「今月まとめて選択」・注意書き・表示制御JS） |
| `src/reservation-routes.js` | 11（特別枠での回数券拒否ガード・予約確認の支払い案内・スタッフ通知🎫・月末リマインド） |
| `src/admin-page.js` | 1（区分バッジ🎫） |
| `tests/kaisuken.test.js` | 8（ファイルごと削除の想定） |

あわせて、参加区分を新体系（都度／ご利用中の方／HMC会員／セミパ会員／相談）に作り直し、**最少催行の前日判定**（HMC3名未満・セミパ2名未満）と**ジャーニーの相談区分**を追加する。
⚠️ **本番反映はデプロイの手（Harnessセッション）の担当。** 司令塔はコードを書いてmainへマージするところまで。
...
- `line-reservation/` の回数券残骸（上の「2. 実装の残り」と同じ話）
...
2. **朝RUNの「予約制」（8/11設計）と「予約不要」（PR #61・8/2公開）が正面から食い違っている。** 上の「オーナーに聞くこと①」参照。LINE予約の改修に入る前に決める
...
5. **謝礼は「1人1回いくら」の従量制。** セミパ¥2,000／HMC¥1,000（講師の紹介客は¥3,000のうち¥2,000）／パーソナル¥3,000。**受け放題と従量謝礼は構造的に噛み合わない**ので、セミパ会員のセミパ月4回上限は外さないこと
