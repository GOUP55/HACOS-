...
| `PRICING_RESTRUCTURE_2026-08.md` | ゲートD4（ジャーニー返金規定）と¥88,000の訂正を追記 |
...
### 2. まだオーナー決定が要る（募集開始のブロッカー）

**① ジャーニー¥78,000の返金・キャンセル規定。**
特商法にこの節だけ書けなかった。でっち上げずに空けてHTMLコメントで印をつけてある。**¥78,000を売り始める前に必須。** 推奨案は `PRICING_RESTRUCTURE_2026-08.md` のゲートD4。

**② 食のサポート（夕食¥8,000／冷凍おかず）の注文キャンセル期限。**
受注生産の生鮮品なので締切が要る。旧 retreat.html に「開催5日前以降は返金不可」というオーナー承認済みの前例があるが、新商品は週次なので決め直しが必要。

**③ 月額の途中加入の回数の数え方**（残り週数で数える案）。日割り自体は確定済み。
...
5. **謝礼は従量制。** セミパ¥2,000／HMC¥1,000（講師の紹介客は¥3,000のうち¥2,000）／パーソナルは**60分¥3,000の時間比例**。**受け放題と従量謝礼は構造的に噛み合わない**ので、セミパ会員のセミパ月4回上限は外さないこと（超過分は¥3,000で受けられるようにした）
