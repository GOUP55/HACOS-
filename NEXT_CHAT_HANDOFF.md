...
# 🔴 最優先（2026-08-11 司令塔セッションからの引き継ぎ）

## いまどこにいるか

**商品体系をゼロから設計し直し、価格・名前・運用ルールがすべて確定した。まだ1文字もLPに反映していない。**
設計の正本は **`PRICING_RESTRUCTURE_2026-08.md`**。冒頭の「📌 最終的な料金」表が最新（それ以外の節は検討の過程なので、食い違ったら冒頭の表が正）。

作業ブランチ: **`claude/august-program-reservation-updates-7i3cle`**（mainより22コミット先行・すべてpush済み・作業ツリーはクリーン）

## このセッションで完了したこと

### 公開物の変更（コミット済み・main未マージ）
- **HMC休講のお知らせをLPに掲載**（`hacos-hmc-lp.html` / `index.html`）。8/2・8/9休講→8/16再開。ヒーロー直下にお知らせ枠を新設（既存にNews枠がなかったため新規）。8/2・8/9カードは休講表示にし予約ボタンと弁当注文リンクを撤去。`2026-08-17 0:00 JST` を過ぎるとJSで自動非表示（コミット `6f64e29`）

### 設計ドキュメント（新規4本）
| ファイル | 内容 |
|---|---|
| `PRICING_RESTRUCTURE_2026-08.md` | **商品体系の設計正本**（934行）。全価格・採算試算・実装ゲート |
| `drafts/kitchen-cost-inquiry.md` | みどりさん・ふみやさんへの原価/キャパ確認文（**送信待ち**） |
| `drafts/journey-3leads-reply.md` | ジャーニー問い合わせ3名への返信文（**送信待ち**） |
| `drafts/journey-positioning.md` | ジャーニーの目的・ことばの整理＋**言い換えが必要な表現3つ** |
...
### 1. 未承認が1つだけ残っている（これだけ先に聞く）
**パーソナルの謝礼を「60分あたり¥3,000」の時間比例にしてよいか。**
現在は「1回¥3,000の固定」だが、それだと**GOは15分・他トレーナーは20分より短いセッションが赤字**になる（10分だとGO −¥1,000／他 −¥1,500）。「10分刻みで時間を選べる」設計にする以上、必ず起きる。時間比例にすれば利益率が一定（GO75%／他67%）になる。

### 2. 実装に着手（承認後）
1. **`tokushoho.html` の書き換え** … 廃止済みHACOSメンバーズが5箇所（170・187・197・206・244行目）残っている。**月額を募集する前に必須**。記載内容は確定済み（解約＝前月末申し出／休会＝可／返金＝日割りなし・月末まで利用可／加入＝日割り）
2. **LPの料金セクションを新体系に差し替え**（`hacos-hmc-lp.html` → `python3 build_index.py`）
3. **`retreat.html` の全面改訂** … 「Wellness Journey」8箇所・「第2期」6箇所を「フィットネスジャーニー」「1期生」に。**URLは変えない**（SEO維持）。商品自体が7日間¥39,800→2ヶ月¥78,000に変わる
4. **LINE予約システムの改修** … 朝RUNを無料枠として予約可能に／参加区分を新体系に／**回数券の全撤去**（`liff/reserve.html`・`src/reservation-routes.js`・`src/admin-page.js`・`tests/kaisuken.test.js`・`BUSINESS_RULES.md`の6項目）／最少催行の前日判定
5. **`BUSINESS_RULES.md` への転記**（設計書はまだ正本ではない）
...
1. **公開中の内容と設計が食い違っている。** `retreat.html` はいま「7DAYS ¥39,800（掲載協力¥34,800）・第2期・締切8/31・9月開催・食事フル込み」で公開中で、LPのヒーローバナーとスティッキーCTAもそこを指している。**問い合わせ中の3名が見ているのはこの内容。** 案内を送るならページの書き換えとセットで
2. **`drafts/journey-positioning.md` の「言い換えが必要な表現」を必ず読む。** オーナー提供のキーワードに、そのままでは公開できない表現が3つある（「リバウンドしないダイエット方法」＝効果の断定／「科学的に実証された…食事療法」＝出典なしの権威付け＋医療用語／「10年後は今よりずっと健康に」＝効果の約束）
3. **食事を真空パックで渡すなら保健所に確認する。** 常温放置でボツリヌス菌などのリスクがあり、要冷蔵/要冷凍の表示・アレルギー表示・消費期限が要る
4. **謝礼は「1人1回いくら」の従量制。** セミパ¥2,000／HMC¥1,000（講師の紹介客は¥3,000のうち¥2,000）／パーソナル¥3,000。**受け放題と従量謝礼は構造的に噛み合わない**ので、セミパ会員のセミパ月4回上限は外さないこと
5. **みどりさんをジャーニー1期生に含める場合**、掲載協力特典を適用するかと、体験談を宣伝に使うなら**スタッフである旨の明記**が必要
6. **`REVENUE_SIMULATION.md` は陳腐化している**（廃止したメンバーズを売上の55〜57%として計算したまま）
