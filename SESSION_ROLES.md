...
| 8/1 | 🧵 **アパレル担当**（本セッション・担当宣言） | `HMC-SOCKS-PROJECT/`（新設）・`APPAREL_ROADMAP.md`・`BUSINESS_RULES.md`（アパレル節追記）。LP・line-reservationには触らない | オーナー発注により第1弾=5本指ソックスの開発資料一式を作成。GOの3決定（予算・初回構成・商品名）待ち |
