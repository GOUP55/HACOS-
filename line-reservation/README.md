...
**`migrations/mig-2026-07-17-admin-ops-log.sql` を本番D1に1回だけ実行してから、Workerをデプロイすること**
（新規テーブルのみ・再実行しても壊れない）。LIFFは無変更のため**KV更新は不要**。

- **開催日の管理**（管理画面の「🛠 開催日の管理」）: 新規追加・編集・受付締切/再開・削除が
  スマホで完結する。月末の来月分登録にMONTHLY.mdのSQLは不要になった（SQLでの運用も引き続き可能）
  - 新規登録のidは日付と同じ（朝クラス用）。TACOS Party等の特別枠は従来どおりSQLで
  - 日付＝IDは変更不可（変えたい場合は削除→新規登録）
  - 削除は予約（キャンセル履歴含む）が1件でもあれば拒否（d1_sessions.cjs removeと同じ規則）。
    お休みにしたい場合は「受付締切」を使う
  - お弁当の選択肢は1行1つ「メニュー名:価格」形式で入力
- **操作ログ**: `admin_ops_log` テーブルに「誰が・いつ・何を」を記録（session_create/update/delete）
- **弁当の個人行表示**（オーナー要望）: 予約者の各行に本人の弁当を表示
  （例: 茉由 🍱ビーフストロガノフ ／ 🏃朝RUN）。集計行と同じ共用パーサを使うため合計は必ず一致
